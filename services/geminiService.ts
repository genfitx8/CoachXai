...
export const generateCoachXChatResponse = async (
  ...
  try {
    ...
    const prompt = `You are CoachX, an AI coaching intelligence assistant embedded in SwingNote — a golf lesson management platform used by professional golf coaches.
...
- Ground your answer in the lesson data provided above wherever relevant.
...
export const generateCoachXInsights = async (
  ...
  try {
    ...
    for (const l of allLessons) {
      ...
      for (const kw of ['슬라이스','훅','어드레스','그립','백스윙','임팩트','체중이동','퍼팅','어프로치','드라이버','아이언','slice','hook','address','grip','backswing','impact','putting','driver']) {
...
Generate exactly 3–5 coaching insights. Each insight must be a JSON object with these fields:
...
- Cover a variety of insight types across the set.
...
[
  ...
  {"type":"attention","title":"High activity members this month","body":"..."}
    ...
    const VALID_TYPES = new Set(['pattern', 'attention', 'curriculum', 'coach_growth', 'stagnation']);
    ...
    return parsed
      .filter(i => i.title && i.body && VALID_TYPES.has(i.type))
      .map(i => ({ ...i, icon: ICON_MAP[i.type] ?? '💡' }));
  } catch (error) {
