...
class ReservationService {
  ...
  async approveReservation(reservationId: string): Promise<LessonReservation> {
    ...
    // Send confirmation notification to the member (only once per approval event).
    // Failures are logged and must not roll back the approval.
    if (!updatedReservation.confirmationNotifiedAt) {
      const notifiedAt = Date.now();
      let notified = false;
      try {
        notified = await notificationService.sendLessonConfirmedNotification(
          updatedReservation
        );
      } catch (err) {
        console.error('[ReservationService] Notification service error during approval:', err);
      }

      if (notified) {
        const withNotificationTimestamp: LessonReservation = {
          ...updatedReservation,
          confirmationNotifiedAt: notifiedAt,
        };
        try {
          if (this.isFirebaseMode()) {
            await firebaseService.updateReservation(withNotificationTimestamp);
          } else {
            storageService.updateReservation(withNotificationTimestamp);
          }
          return withNotificationTimestamp;
        } catch (err) {
          console.error('[ReservationService] Failed to persist confirmationNotifiedAt:', err);
        }
      }
    }
