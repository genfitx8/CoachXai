...
/**
 * Returns a shareable URL that deep-links directly to the lesson record.
 * The URL uses a hash fragment (`#lesson=<id>`) so the SPA can detect it
 * on load and navigate the user to the correct lesson automatically.
 */
export function buildLessonShareUrl(lesson: Lesson): string {
  return `${window.location.origin}/#lesson=${encodeURIComponent(lesson.id)}`;
}
...
export async function sendLessonNoteViaKakao(
  ...
  const pageUrl = buildLessonShareUrl(lesson);
