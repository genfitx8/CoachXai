...
import { Lesson, ClientProfile, Homework, HomeworkTemplate, NotificationMessage, GolfCourse, CoachProfile, LessonReservation, CalendarIntegration, Branch, BranchAdminAccount, Bay, BayPriceRule, BayReservation, LessonPackage, TrainingProgram, QuickLogEntry, WeeklyInsight, CoachMaterial } from '../types';
...
const STORAGE_KEYS = {
  ...
  COACH_MATERIALS: 'swingnote_coach_materials',
};
...
export const storageService = {
  ...
  // ── Coach Material Methods ──────────────────────────────────────────────────

  getCoachMaterials: (): CoachMaterial[] => {
    try {
      const data = localStorage.getItem(STORAGE_KEYS.COACH_MATERIALS);
      return data ? JSON.parse(data) : [];
    } catch (e) {
      console.error('Failed to load coach materials', e);
      return [];
    }
  },

  saveCoachMaterial: (material: CoachMaterial): void => {
    try {
      const all = storageService.getCoachMaterials();
      const idx = all.findIndex((m) => m.id === material.id);
      const updated =
        idx >= 0
          ? [...all.slice(0, idx), material, ...all.slice(idx + 1)]
          : [...all, material];
      localStorage.setItem(STORAGE_KEYS.COACH_MATERIALS, JSON.stringify(updated));
    } catch (e) {
      console.error('Failed to save coach material', e);
    }
  },

  deleteCoachMaterial: (materialId: string): void => {
    try {
      const all = storageService.getCoachMaterials();
      localStorage.setItem(
        STORAGE_KEYS.COACH_MATERIALS,
        JSON.stringify(all.filter((m) => m.id !== materialId))
      );
    } catch (e) {
      console.error('Failed to delete coach material', e);
    }
  },
};
