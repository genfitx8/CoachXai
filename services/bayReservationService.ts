...
export const bayReservationService = {
   ...
   * Uses the same validation logic as createReservation but deducts points
   * from the coach's profile (stored in the coaches collection, not clients).
   * The reservation is stored with clientId = coachProfile.id to distinguish
   * coach reservations from regular client reservations.
   */
  createCoachBayReservation: async (params: {
    ...
    // Build deterministic ID using coach.id and check double-booking
    const ymd = toYMD(date);
    const hh = String(startHour).padStart(2, '0');
    const reservationId = `${branch.id}_${bay.id}_${ymd}_${hh}`;
    ...
    // Store coach reservations with coach.id as the clientId
    const reservation: BayReservation = {
      id: reservationId,
      branchId: branch.id,
      bayId: bay.id,
    ...
    // Deduct points from coach profile and persist to coaches collection
    const newPoints = coachPoints - rule.pricePoints;
    const updatedCoach: CoachProfile = { ...coach, currentPoints: newPoints };
    try {
      if (firebaseService.isInitialized()) {
        await firebaseService.saveCoach(updatedCoach);
      } else {
        localStorage.setItem('swingnote_coach_profile', JSON.stringify(updatedCoach));
      }
    } catch (pointError) {
      // Rollback reservation on point persistence failure
      await updateReservation(reservationId, { status: 'CANCELLED' as BayReservationStatus });
      throw new Error('포인트 차감에 실패했습니다. 예약이 취소되었습니다.');
    }

    // Fire-and-forget: send push notifications after successful reservation.
    sendBayReservationNotifications(reservation, branch.name).catch((e) =>
      console.error('[BayReservationService] Unexpected notification error:', e)
    );
