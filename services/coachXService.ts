...
function buildCurriculum5(
  ...
  const t0 = recentTopics[0] ?? '어드레스 & 셋업';
  const t1 = recentTopics[1] ?? '임팩트 감각';
  const t2 = recentTopics[2] ?? '클럽패스';
...
export function buildMemberGrowthReports(
  ...
  return Object.entries(clientMap).map(([key, lessons]) => {
    ...
    const curriculumPlan5 = buildCurriculum5(recentTopics, repeatedIssues, trend);
