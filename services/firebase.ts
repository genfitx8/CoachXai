...
export const firebaseService = {
  ...
  // ── Lesson Package Operations ───────────────────────────────────────────────

  getLessonPackages: async (coachId: string): Promise<LessonPackage[]> => {
    if (!db) return [];
    try {
      const q = query(
        collection(db, 'lesson_packages'),
  ...
  saveLessonPackage: async (pkg: LessonPackage): Promise<void> => {
    if (!db) throw new Error('Firebase not initialized');
    const cleaned = removeUndefinedFields(pkg);
    await setDoc(doc(db, 'lesson_packages', pkg.id), cleaned);
  },

  deleteLessonPackage: async (packageId: string): Promise<void> => {
    if (!db) throw new Error('Firebase not initialized');
    await deleteDoc(doc(db, 'lesson_packages', packageId));
  },

  // ── Training Program Operations ─────────────────────────────────────────────

  getTrainingPrograms: async (coachId: string): Promise<TrainingProgram[]> => {
    if (!db) return [];
    try {
      const q = query(
  ...
  saveTrainingProgram: async (program: TrainingProgram): Promise<void> => {
    if (!db) throw new Error('Firebase not initialized');
    const cleaned = removeUndefinedFields(program);
    await setDoc(doc(db, 'training_programs', program.id), cleaned);
  },

  deleteTrainingProgram: async (programId: string): Promise<void> => {
    if (!db) throw new Error('Firebase not initialized');
    await deleteDoc(doc(db, 'training_programs', programId));
  },

  // ── Quick Log Operations ─────────────────────────────────────────────────────

  saveQuickLog: async (entry: QuickLogEntry): Promise<void> => {
    if (!db) throw new Error('Firebase not initialized');
    const cleaned = removeUndefinedFields(entry);
    await setDoc(doc(db, 'quick_logs', entry.id), cleaned);
  },

  getQuickLogsByClient: async (clientId: string): Promise<QuickLogEntry[]> => {
    if (!db) return [];
    try {
      const q = query(
        collection(db, 'quick_logs'),
        where('clientId', '==', clientId),
        orderBy('createdAt', 'desc')
      );
      const snap = await getDocs(q);
  ...
  deleteQuickLog: async (logId: string): Promise<void> => {
    if (!db) throw new Error('Firebase not initialized');
    await deleteDoc(doc(db, 'quick_logs', logId));
  },

  // ── Weekly Insight Operations ────────────────────────────────────────────────

  saveWeeklyInsight: async (insight: WeeklyInsight): Promise<void> => {
    if (!db) throw new Error('Firebase not initialized');
    const cleaned = removeUndefinedFields(insight);
    await setDoc(doc(db, 'weekly_insights', insight.id), cleaned);
  },

  getWeeklyInsightsByClient: async (clientId: string): Promise<WeeklyInsight[]> => {
    if (!db) return [];
    try {
      const q = query(
        collection(db, 'weekly_insights'),
        where('clientId', '==', clientId),
  ...
  // ── Coach Material Operations ───────────────────────────────────────────────

  getCoachMaterialsByCoach: async (coachId: string): Promise<CoachMaterial[]> => {
    if (!db) return [];
    try {
      const q = query(
        collection(db, 'coach_materials'),
        where('coachId', '==', coachId)
      );
      const snap = await getDocs(q);
      return snap.docs.map((d) => d.data() as CoachMaterial);
    } catch (e) {
      console.error('Failed to fetch coach materials by coach:', e);
      return [];
    }
  },

  getCoachMaterialsByClient: async (clientId: string): Promise<CoachMaterial[]> => {
    if (!db) return [];
    try {
      const q = query(
        collection(db, 'coach_materials'),
      ...
      console.error('Failed to fetch coach materials by client:', e);
      return [];
    }
  },

  saveCoachMaterial: async (material: CoachMaterial): Promise<void> => {
    if (!db) throw new Error('Firebase not initialized');
    const cleaned = removeUndefinedFields(material);
    await setDoc(doc(db, 'coach_materials', material.id), cleaned);
  },

  deleteCoachMaterial: async (materialId: string): Promise<void> => {
    if (!db) throw new Error('Firebase not initialized');
    await deleteDoc(doc(db, 'coach_materials', materialId));
  },

  // ── Lesson Inquiry Operations ───────────────────────────────────────────────

  saveLessonInquiry: async (inquiry: LessonInquiry): Promise<void> => {
    if (!db) throw new Error('Firebase not initialized');
    const cleaned = removeUndefinedFields(inquiry);
    await setDoc(doc(db, 'lesson_inquiries', inquiry.id), cleaned);
  },

  getLessonInquiriesByClient: async (clientId: string): Promise<LessonInquiry[]> => {
    if (!db) return [];
    try {
      const q = query(
        collection(db, 'lesson_inquiries'),
        where('clientId', '==', clientId),
        orderBy('createdAt', 'desc')
      );
      const snap = await getDocs(q);
      return snap.docs.map((d) => d.data() as LessonInquiry);
    } catch (e) {
      console.error('Failed to fetch lesson inquiries by client:', e);
      return [];
    }
  },

  getLessonInquiriesByCoach: async (coachId: string): Promise<LessonInquiry[]> => {
    if (!db) return [];
    try {
      const q = query(
        collection(db, 'lesson_inquiries'),
      ...
      console.error('Failed to fetch lesson inquiries by coach:', e);
      return [];
    }
  },
};
