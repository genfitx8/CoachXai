...
export const firebaseService = {
  ...
  savePromptTemplate: async (template: PromptTemplate): Promise<void> => {
    ...
    if (template.isActive) {
      try {
        ...
        snap.docs.forEach((d) => {
          if (d.id !== template.id) {
            batch.update(d.ref, { isActive: false, updatedAt: Date.now() });
          }
        });
        await batch.commit();
      } catch (e) {
  ...
  /**
   * Upload a file to Firebase Storage under `prompt_attachments/{promptId}/{attachmentId}`.
   * Returns the download URL.
   */
  uploadPromptAttachment: async (
    promptId: string,
    attachmentId: string,
    file: File
  ): Promise<{ storagePath: string; downloadUrl: string }> => {
    if (!storage) throw new Error('Firebase Storage not initialized');
    const storagePath = `prompt_attachments/${promptId}/${attachmentId}_${file.name}`;
  ...
  deletePromptAttachmentFile: async (storagePath: string): Promise<void> => {
    if (!storage) return;
    try {
      const storageRef = ref(storage, storagePath);
  ...
  savePromptAttachment: async (
    promptId: string,
    attachment: PromptAttachment
  ): Promise<void> => {
    if (!db) throw new Error('Firebase not initialized');
    const docRef = doc(db, 'prompt_templates', promptId);
    const snap = await getDoc(docRef);
    if (!snap.exists()) throw new Error('Prompt template not found');
    const template = snap.data() as PromptTemplate;
    const existingIdx = template.attachments.findIndex((a) => a.id === attachment.id);
    const newAttachments =
      existingIdx >= 0
        ? template.attachments.map((a) => (a.id === attachment.id ? attachment : a))
  ...
  deletePromptAttachmentRecord: async (
    promptId: string,
    attachmentId: string
  ): Promise<void> => {
    if (!db) throw new Error('Firebase not initialized');
    ...
    const template = snap.data() as PromptTemplate;
    const newAttachments = template.attachments.filter((a) => a.id !== attachmentId);
    await updateDoc(docRef, {
      attachments: newAttachments,
      updatedAt: Date.now(),
    });
