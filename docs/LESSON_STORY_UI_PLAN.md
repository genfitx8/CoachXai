...
1. **대표컷 선정** — 우선순위:
   `story.coverMediaId` (코치 지정) → `role==='AFTER'` 영상 → `thumbnailUrl` →
   `additionalMedia` 중 `source==='live_lesson'` 첫 이미지 → `videoUrl`.
2. **본문 미디어 풀** = `reviewSections.attachmentIds` 에 포함된 미디어에서
   대표컷과 Before/After 쌍을 제외한 것. (검토 화면에서 코치가 이미 "이건
   붙일 것"이라 골라둔 목록이므로 새 UI 없이 의도를 재사용한다.)
3. **교차 삽입** — 문단 `N`개, 미디어 `M`개일 때 간격
   `gap = max(1, ceil(N / (M + 1)))` 로 문단 `gap, 2·gap, …` 뒤에 하나씩 꽂는다.
   - `N < M` 이면 남은 미디어는 하단 갤러리 그리드로 몰아준다.
   - 연속 두 미디어 사이에는 최소 문단 1개를 보장한다(사진이 붙어 나오면
     다시 갤러리처럼 보인다).
...
| | 범위 | 산출 | 검증 |
| --- | --- | --- | --- |
| **M1** | 자동 조판 + 읽기 전용 스토리 뷰 + 페이퍼 테마 | 기존 필드만으로 모든 기록이 스토리로 보인다 | `composeStory` 단위 테스트: 문단/미디어 교차, 빈 데이터, MEDIA_ONLY, 구 기록 폴백 |
| **M2** | `generateLessonStoryMeta` — 헤드라인·캡션·대표컷 | 승인 시 1회 생성, 실패 시 폴백 | AI 미가동 상태에서도 M1과 동일하게 렌더 |
| **M3** | 코치 저작 (헤드라인/캡션/순서/대표컷) | `story.blocks` 저장 | 저장된 순서가 재조판에 덮이지 않음 |
