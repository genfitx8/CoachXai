...
## 12.8 M3a 구현 결과 — 코치가 스토리 위에서 직접 쓴다

M3 의 저작 기능 중 **글자를 고치는 부분**을 먼저 넣었다. 기획의 M3 목록에서
두 항목은 상황이 달라졌다.

**대표컷 교체는 항목 자체가 사라졌다** — §12.7 에서 표지에 사진을 걸지 않기로
했으므로 고를 대표컷이 없다.

**사진 순서 이동은 보류했다** — §12.7 의 시각 기준 배치가 이미 "이 사진이
어느 이야기에 속하는가"를 정한다. 배치가 실제로 어긋나는 것을 보기 전에
순서 이동을 만들면 쓰지 않을 기능이 된다. 필요해지면 `story.blocks` 를
통째로 저장하는 대신 **자리 override**(`mediaId → 문단 index`)로 넣는 편이
낫다 — 그래야 코치가 나중에 본문을 고쳐도 스토리가 따라 바뀐다.

### 어디서 고치는가

기획(§8)은 승인 전 검토 화면에 "스토리 미리보기"를 띄우고 그 안에서 고치게
했는데, **스토리 탭에서 바로 고치는 쪽**으로 바꿨다. 코치는 이미 완성된
스토리를 읽고 있고, 어색한 제목을 발견하는 것도 그 화면에서다. 별도 편집
화면으로 보내면 한 단계가 늘고, 승인 후에 오타를 고칠 길도 없어진다.

`components/story/StoryEditable.tsx` — 주변 타이포를 통째로 물려받아 편집
중에도 글이 흔들리지 않는다. 종이 위에 입력 상자를 그리면 읽는 화면이 아니라
폼처럼 보이므로, 평소에는 아주 옅은 밑줄만 남긴다.

| 자리 | 최대 | 안내 문구 |
| --- | --- | --- |
| 제목 | 40자 | 그날의 한 줄을 적어보세요 |
| 리드 | 80자 | 한 줄 덧붙이기 (없어도 됩니다) |
| 사진 캡션 | 40자 | ＋ 캡션 달기 |

**학생 화면에는 편집 자국이 없다.** 빈 리드는 아예 없는 것으로 그려지고,
캡션이 없는 사진은 지금처럼 빈 여백만 남는다.

### 두 가지 세부

**`editedByCoach` 를 함께 올린다** — 코치가 손댄 스토리를 AI 가 덮으면 안
된다. `generateLessonStoryMeta()` 가 이 값과 헤드라인을 함께 보고 건너뛴다.

**빈 캡션은 키를 지운다** — 빈 문자열을 남기면 "고쳤는데 비웠다"와 "아직 안
썼다"가 구분되지 않아 다음 조판이 빈 캡션을 그린다.

### 남은 것 — M3b: AI 캡션 생성

M2(§12.6)에서 미룬 캡션 생성은 이제 안전하게 붙일 수 있다. 코치가 캡션을
보고 고칠 자리가 생겼으므로, AI 가 제안하고 코치가 승인하는 순서가 성립한다.
사진을 실제로 보는 vision 호출이 필요하므로 비용·지연을 함께 재야 한다.
...
| **M3a** ✅ | 코치 저작 — 제목·리드·캡션 인라인 편집 | `story` patch 저장 + `editedByCoach` | 완료 — `LessonStoryAuthoring.test.tsx` 12개(학생 화면 무자국, 저장/취소, 빈 캡션 키 삭제) |
| **M3b** | AI 캡션 생성 (vision) | 코치가 보고 고친 뒤에만 학생에게 나간다 | — |
