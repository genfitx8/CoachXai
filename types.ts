...
export interface PointTransaction {
  ...
  type: 'HOMEWORK' | 'LESSON_RECORD' | 'PURCHASE' | 'ADJUSTMENT' | 'POINT_TOPUP_TOSS' | 'BRANCH_ADMIN_GRANT';
  description: string;
...
export type ViewState = 'LIST' | 'DETAIL' | 'NEW' | 'COMPARE' | 'CLIENTS' | 'CLIENT_STATS' | 'LESSON_LIST' | 'LESSON_PACKAGE' | 'TRAINING_PROGRAM' | 'COACH_MATERIAL';

/** Type of material the coach is producing for a client. */
export type CoachMaterialType =
  | 'LESSON_GUIDE'
  | 'DRILL_SHEET'
  | 'SWING_TIPS'
  | 'COURSE_STRATEGY'
...
/**
 * A coach-authored material document generated (and optionally AI-drafted)
 * for a specific client based on their lesson history.
 */
export interface CoachMaterial {
  ...
  clientPhone: string;
  title: string;
  type: CoachMaterialType;
  /** AI-generated or hand-edited material content in markdown format. */
  content: string;
  /** The goal or request entered by the coach when generating the material. */
  goal: string;
  /** draft = saved but not yet shared; published = visible to the client. */
  status: 'draft' | 'published';
  /** IDs of the lessons that were used as context for generation. */
  lessonIds?: string[];
  createdAt: number;
  updatedAt: number;
}
...
export interface CoachProfile {
  ...
  // Location / finder fields (optional – used by CoachFinder)
  region?: string; // 활동 지역 e.g. "서울 강남구"
  introduction?: string; // 코치 소개 한줄
  isLessonAvailable?: boolean; // 레슨 가능 여부
}
...
// ─── Coach Finder & Lesson Inquiry ───────────────────────────────────────────

/** Lesson inquiry submitted by a member to a specific coach. */
...
/** Result type returned by CoachFinderService (extends CoachProfile with distance info). */
export interface CoachFinderResult extends CoachProfile {
  distanceKm?: number; // approximate distance from search origin (undefined when region-based)
}
