...
export const MembershipPurchase: React.FC<MembershipPurchaseProps> = ({
  ...
  const phone = (clientProfile.phone ?? '').replace(/[^0-9]/g, '');
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      if (!orderRes.ok) {
        const data = await orderRes.json().catch(async () => ({
          error: await orderRes.text().catch(() => ''),
        }));
        const serverError = (data as { error?: string }).error?.trim();
        throw new Error(
          serverError
            ? `멤버십 주문 생성 실패: ${serverError}`
            : `멤버십 주문 생성 실패 (HTTP ${orderRes.status})`
        );
      }
