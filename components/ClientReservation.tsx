...
import { ClientProfile, LessonReservation, ReservationStatus, CoachProfile } from '../types';
...
export const ClientReservation: React.FC<ClientReservationProps> = ({ clientProfile, onBack }) => {
  ...
  const [view, setView] = useState<'AVAILABLE' | 'MY_RESERVATIONS' | 'CALENDAR'>('CALENDAR');
  const [selectedSlot, setSelectedSlot] = useState<LessonReservation | null>(null);
  const [notes, setNotes] = useState('');
  const [filterStatus, setFilterStatus] = useState<ReservationStatus | 'ALL'>('ALL');
  
  // New form state for direct time input
              ...
              <div className="bg-white rounded-lg shadow-sm p-4">
                <div className="mb-4">
                  <h3 className="text-lg font-semibold text-gray-900 mb-2">
                    {clientProfile.designatedCoach} 코치 스케줄
                  </h3>
                  <p className="text-sm text-gray-600">
                    예약 가능한 시간(파란색)을 클릭하여 바로 예약할 수 있습니다.
                  </p>
                </div>
                <CalendarView 
                  coachProfile={{
                    id: coachId,
                    name: clientProfile.designatedCoach || '',
                    email: '',
                    phone: '',
                    specialization: '',
                    bio: ''
                  ...
                  clientProfile={clientProfile}
                  readOnly={false}
                  onBookingComplete={loadMyReservations}
                />
              </div>
            )}
          </div>
