    ...
    <div className="flex gap-2 items-end">
      {counts.map((c, i) => (
        <div key={i} className="flex-1 flex flex-col items-center gap-0.5">
          <span className="text-[10px] text-violet-700 font-semibold h-4 flex items-end justify-center">
            {c.count > 0 ? c.count : ''}
          </span>
          <div
        ...
        <div className="border-t border-gray-100">
            ...
            <div className="px-4 pb-4 pt-3 space-y-4">
              ...
              <div>
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2.5 flex items-center gap-1">
                  <Calendar className="w-3 h-3 text-violet-500" /> {t('coachx_history_monthly_activity')}
                </p>
              ...
              <div>
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2.5 flex items-center gap-1">
                  <BookOpen className="w-3 h-3 text-indigo-500" /> {t('coachx_history_recent_lessons')}
                </p>
                    ...
                    {memberLessons.slice(0, 8).map((lesson) => (
                      <li key={lesson.id} className="flex gap-3 items-start">
                        ...
                        <div className="flex-1 pb-1">
                          <p className="text-[11px] font-semibold text-violet-600">{lesson.date}</p>
          ...
          <div className="border-t border-gray-50 px-4 py-3 flex gap-2">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-1.5 text-xs font-semibold text-indigo-700 bg-indigo-50 hover:bg-indigo-100 active:bg-indigo-200 rounded-lg py-2.5 transition-colors"
            >
            ...
            <button
              ...
              className="flex-1 flex items-center justify-center gap-1.5 text-xs font-semibold text-violet-700 bg-violet-50 hover:bg-violet-100 active:bg-violet-200 rounded-lg py-2.5 transition-colors"
            >
...
const CoachGrowthTab: React.FC<{ profile: CoachGrowthProfile; loading?: boolean }> = ({ profile, loading = false }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ── Gemini AI Summary ──────────────────────────────────────────────── */}
      {loading ? (
        <div className="bg-gradient-to-br from-violet-50 to-indigo-50 rounded-xl border border-violet-100 p-4 flex items-center gap-2">
          <Sparkles className="w-4 h-4 text-violet-400 animate-pulse flex-shrink-0" />
          <p className="text-xs text-violet-500 italic">{t('coachx_growth_ai_summary_loading')}</p>
        </div>
      ) : profile.geminiSummary ? (
        <div className="bg-gradient-to-br from-violet-50 to-indigo-50 rounded-xl border border-violet-100 p-4">
          <p className="text-xs font-semibold text-violet-700 uppercase tracking-wide flex items-center gap-1.5 mb-2">
            <Sparkles className="w-3.5 h-3.5" />
              ...
              return (
                <li key={i} className="flex items-start gap-3 px-4 py-3">
                  ...
                  <button
                    ...
                    className="flex-shrink-0 text-xs font-semibold text-violet-600 hover:text-violet-700 bg-violet-50 hover:bg-violet-100 rounded-lg px-2.5 py-1.5 transition-colors"
                  >
