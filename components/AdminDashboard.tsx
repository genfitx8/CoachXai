...
import { Users, FileText, Trash2, LogOut, RefreshCcw, ShieldAlert, CreditCard, CheckCircle, XCircle, Activity, LayoutDashboard, UserX, Database, Video, Mic, Image as ImageIcon, User, Award, Calendar, Cloud, Save, AlertTriangle, ListChecks, Plus, Send, Bell, MessageSquare, Megaphone, MapPin, Briefcase, Search } from 'lucide-react';
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'CLIENTS' | 'LESSONS' | 'SYSTEM' | 'TEMPLATES' | 'MESSAGES' | 'COURSES'>('CLIENTS');
        ...
        <div className="animate-fade-in">
                        ...
                        {memberType === 'GENERAL' ? (
                            <table className="w-full text-sm text-left">
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {clients.length === 0 ? (
                                        <tr><td colSpan={5} className="px-6 py-8 text-center text-gray-400">등록된 일반 회원이 없습니다.</td></tr>
                                    ) : (
                                        clients.map((client) => (
                            ...
                            <table className="w-full text-sm text-left">
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {coaches.length === 0 ? (
                                        <tr><td colSpan={5} className="px-6 py-8 text-center text-gray-400">등록된 코치 회원이 없습니다.</td></tr>
                                    ) : (
                                        coaches.map((coach) => {
                        ...
                        <table className="w-full text-sm text-left">
                            <thead className="bg-gray-50 text-gray-500 font-medium border-b border-gray-200">
                                <tr>
                                    ...
                                    <th className="px-6 py-3">미디어</th>
                                    <th className="px-6 py-3 text-right">관리</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-100">
                                {lessons.length === 0 ? (
                                    <tr><td colSpan={6} className="px-6 py-8 text-center text-gray-400">등록된 레슨이 없습니다.</td></tr>
                                ) : (
                                    lessons.sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map((lesson) => (
                                        <tr key={lesson.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-gray-500"><span className="flex items-center gap-1 capitalize">{getMediaIcon(lesson.mediaType)} {lesson.mediaType}</span></td>
                                            <td className="px-6 py-4 text-right">
            ...
            {activeTab === 'TEMPLATES' && (
                <div className="space-y-4">
                    <div className="flex items-center justify-between">
                        <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                            <ListChecks className="w-5 h-5" /> 과제 템플릿 관리
                        </h2>
                    </div>
            ...
            {activeTab === 'MESSAGES' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                        <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                            <Megaphone className="w-5 h-5" /> 푸시 메시지 전송
                        </h2>
                            ...
                            <div>
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button 
                                        ...
                                        className={`flex-1 py-2 text-sm font-bold rounded-lg border ${msgTarget === 'COACHES' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-600 border-gray-200'}`}
                                    >
                                        코치 전체
                                    </button>
                                    <button 
                                        onClick={() => setMsgTarget('CLIENTS')}
                                        className={`flex-1 py-2 text-sm font-bold rounded-lg border ${msgTarget === 'CLIENTS' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-gray-600 border-gray-200'}`}
                                    >
                                        회원 전체
                                    </button>
                                </div>
                                        ...
                                        <div key={msg.id} className="p-4 hover:bg-gray-50 transition-colors">
                                            <div className="flex justify-between items-start mb-1">
                                                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                                                    ...
                                                    {msg.target === 'ALL' ? '전체 발송' : msg.target === 'COACHES' ? '코치 전용' : '회원 전용'}
                                                </span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 h-fit md:col-span-2">
                        ...
                        <textarea
                            ...
                            className="w-full h-40 p-3 font-mono text-xs border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 mb-4"
                        />
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 h-fit">
                        <h2 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2"><CreditCard className="w-5 h-5" /> 구독 관리</h2>
                                ...
                                {subscriptionSearchResults.length === 0 ? (
                                    ...
                                    subscriptionSearchResults.map((user, idx) => (
                                        ...
                                        >
                                            ...
                                            {user.isSubscribed && <CheckCircle className="w-3 h-3 text-emerald-500" />}
                                        </div>
                                    ))
                                )}
                            </div>
