...
export const PointHistoryModal: React.FC<PointHistoryModalProps> = ({ isOpen, onClose, client }) => {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
      <div className="bg-white/[0.04] rounded-2xl w-full max-w-md overflow-hidden shadow-2xl flex flex-col max-h-[80vh]">
        ...
        <div className="p-4 border-b border-line-subtle">
            <a 
                ...
                className="flex items-center justify-center gap-2 w-full py-3 bg-base text-white rounded-xl font-bold hover:bg-black transition-colors shadow-lg shadow-gray-200"
            >
        ...
        {/* List */}
        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-white/[0.03]">
            <h3 className="text-sm font-bold text-ink-muted mb-3 px-2">최근 내역</h3>
            
            {isLoading ? (
                <div className="text-center py-10 text-ink-muted">불러오는 중...</div>
            ) : history.length === 0 ? (
                <div className="text-center py-10 text-ink-muted text-sm">아직 적립된 포인트가 없습니다.</div>
            ) : (
                <div className="space-y-3">
                    {history.map(item => (
                        <div key={item.id} className="bg-white/[0.04] p-4 rounded-xl shadow-sm border border-line-subtle flex items-center justify-between">
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    <p className="font-bold text-ink-high text-sm">{item.description}</p>
