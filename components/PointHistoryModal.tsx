...
export const PointHistoryModal: React.FC<PointHistoryModalProps> = ({ isOpen, onClose, client }) => {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
      <div className="bg-bg-raised rounded-2xl w-full max-w-md overflow-hidden shadow-2xl flex flex-col max-h-[80vh]">
        ...
        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-bg-base">
            <h3 className="text-sm font-bold text-ink-medium mb-3 px-2">최근 내역</h3>
            ...
            {isLoading ? (
                ...
                <div className="space-y-3">
                    {history.map(item => (
                        <div key={item.id} className="bg-bg-raised p-4 rounded-xl shadow-sm border border-line-subtle flex items-center justify-between">
                            <div className="flex items-start gap-3">
                                <div className={`mt-1 p-2 rounded-full ${item.amount > 0 ? 'bg-primary-500/15 text-emerald-600' : 'bg-red-500/15 text-red-500'}`}>
