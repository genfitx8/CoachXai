...
import { Calendar, Clock, User, Phone, CheckCircle, XCircle, Plus, ArrowLeft, Filter, Ban, Settings, Save, ChevronLeft, ChevronRight } from 'lucide-react';
...
export const ReservationManager: React.FC<ReservationManagerProps> = ({
  ...
  const [addingHours, setAddingHours] = useState<Set<number>>(new Set());
  ...
  const handleAddHourSlot = async (hour: number) => {
    if (addingHours.has(hour)) return;
    setAddingHours((prev) => new Set(prev).add(hour));
    try {
      ...
      alert(err.message || '슬롯 생성에 실패했습니다.');
    } finally {
      setAddingHours((prev) => {
                ...
                <>
                  ...
                  <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-2">
                    {hourRange.map((hour) => {
                      ...
                      const isAdding   = addingHours.has(hour);
                      const reservation = reservationsForDate.find(
                        (r) => new Date(r.startTime).getHours() === hour
                      );
                      const statusBg = isOccupied
                        ? reservation?.status === 'PENDING'   ? 'bg-yellow-100 border-yellow-300 text-yellow-700'
                        : reservation?.status === 'CONFIRMED' ? 'bg-green-100 border-green-300 text-green-700'
                        : reservation?.status === 'BLOCKED'   ? 'bg-red-100 border-red-300 text-red-700'
                        :                                        'bg-blue-100 border-blue-300 text-blue-700'
                        : 'bg-white border-gray-200 text-gray-700 hover:bg-blue-50 hover:border-blue-400 cursor-pointer';

                      return (
                        <button
                          key={hour}
                          disabled={isOccupied || isAdding}
                          onClick={() => handleAddHourSlot(hour)}
                          title={
                            isOccupied
                              ? `${getStatusText(reservation?.status as ReservationStatus)} (${reservation?.clientName || ''})`
                              : `${String(hour).padStart(2, '0')}:00 슬롯 추가`
                          }
              ...
              {showBlockForm && (
                <form onSubmit={handleBlockSlot} className="space-y-3">
                  ...
                  <input
                    ...
                    placeholder="블럭 사유 (선택)"
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm"
                  />
                  <div className="flex gap-2">
                    <Button type="submit" className="flex-1 text-sm py-2">블럭하기</Button>
                    <button
                      type="button"
