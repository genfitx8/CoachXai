...
import { User, LogOut, History, PlayCircle, Plus, BarChart3, Bell, Sparkles, ListChecks, Globe, Calendar, Search, Filter, Eye, EyeOff, ChevronRight, ChevronLeft, TrendingUp, Award, Target, ClipboardList, ShoppingCart, PenLine } from 'lucide-react';
...
export const ClientApp: React.FC<ClientAppProps> = ({ clientProfile, allLessons, onLogout, onUpdateLesson, onSaveNewRecord, onDeleteLesson, onUpdateProfile }) => {
  ...
  const [view, setView] = useState<ViewState | 'STATS' | 'PROFILE' | 'RESERVATION' | 'CALENDAR_INTEGRATION' | 'BAY_RESERVATION' | 'MY_BAY_RESERVATIONS' | 'POINT_PURCHASE' | 'PAYMENT_SUCCESS' | 'PAYMENT_FAIL' | 'RECENT_RECORDS' | 'AI_AGENT' | 'QUICK_LOG' | 'WEEKLY_INSIGHT'>(() => {
    // Check if we're returning from a Toss payment redirect
    const params = new URLSearchParams(window.location.search);
    if (params.get('paymentKey') && params.get('orderId')) return 'PAYMENT_SUCCESS';
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans pb-20">
      ...
      <main className="max-w-md mx-auto px-4 py-6 space-y-6">
                ...
                <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                    ...
                    <div className="grid grid-cols-4 gap-2">
                        <button
                            ...
                            className="flex flex-col items-center gap-1.5 py-3 px-1 bg-blue-50 hover:bg-blue-100 rounded-xl transition-colors"
                        >
                            <Calendar className="w-5 h-5 text-blue-600" />
                            <span className="text-[10px] font-bold text-blue-700 text-center leading-tight">레슨 예약</span>
                        </button>
                        <button
                        ...
                        >
                            ...
                            <span className="text-[10px] font-bold text-blue-700 text-center leading-tight">타석 예약</span>
                        </button>
                        <button
                        ...
                        >
                            ...
                            <span className="text-[10px] font-bold text-blue-700 text-center leading-tight">예약 내역</span>
                        </button>
                        <button
                        ...
                        >
                            ...
                            <span className="text-[10px] font-bold text-blue-700 text-center leading-tight">캘린더 연동</span>
                        </button>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        <button
                            ...
                            className="flex flex-col items-center gap-1.5 py-3 px-1 bg-emerald-50 hover:bg-emerald-100 rounded-xl transition-colors"
                        >
                            <History className="w-5 h-5 text-emerald-600" />
                            <span className="text-[10px] font-bold text-emerald-700 text-center leading-tight">{t('recent_records')}</span>
                        </button>
                        <button
                        ...
                        >
                            ...
                            <span className="text-[10px] font-bold text-emerald-700 text-center leading-tight">상세 통계</span>
                        </button>
                        <button
                            onClick={() => { setSelectedLesson(null); setView('WEEKLY_INSIGHT'); }}
                            className="flex flex-col items-center gap-1.5 py-3 px-1 bg-purple-50 hover:bg-purple-100 rounded-xl transition-colors"
                        >
                            <TrendingUp className="w-5 h-5 text-purple-600" />
                            <span className="text-[10px] font-bold text-purple-700 text-center leading-tight">주간 인사이트</span>
                        </button>
                        <button
                            onClick={() => { setSelectedLesson(null); setView('AI_AGENT'); }}
                            className="flex flex-col items-center gap-1.5 py-3 px-1 bg-indigo-50 hover:bg-indigo-100 rounded-xl transition-colors"
                        >
                            <Sparkles className="w-5 h-5 text-indigo-600" />
                            <span className="text-[10px] font-bold text-indigo-700 text-center leading-tight">AI 코치</span>
                        </button>
                        <button
                        ...
                        >
                            ...
                            <span className="text-[10px] font-bold text-emerald-700 text-center leading-tight">빠른 기록</span>
                        </button>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <button
                            ...
                            className="flex flex-col items-center gap-1.5 py-3 px-1 bg-gray-50 hover:bg-gray-100 rounded-xl transition-colors"
                        >
                            <User className="w-5 h-5 text-gray-600" />
                            <span className="text-[10px] font-bold text-gray-700 text-center leading-tight">내 정보</span>
                        </button>
                        <button
                            onClick={() => { setSelectedLesson(null); setView('POINT_PURCHASE'); }}
                            className="flex flex-col items-center gap-1.5 py-3 px-1 bg-yellow-50 hover:bg-yellow-100 rounded-xl transition-colors"
                        >
                            <ShoppingCart className="w-5 h-5 text-yellow-600" />
                            <span className="text-[10px] font-bold text-yellow-700 text-center leading-tight">포인트 구매</span>
                        </button>
                    </div>
                </div>
        ...
        {view === 'PAYMENT_FAIL' && (
            <PaymentFail onBack={() => setView('POINT_PURCHASE')} />
        )}
