...
import { Lesson, ViewState, ClientProfile, CoachProfile, Homework, QuickLogEntry, CoachMaterial } from '../types';
...
import { User, LogOut, History, PlayCircle, Plus, BarChart3, Bell, Sparkles, ListChecks, Globe, Calendar, Search, Filter, Eye, EyeOff, ChevronRight, ChevronLeft, TrendingUp, Award, Target, ClipboardList, ShoppingCart, PenLine, BookOpen, MapPin } from 'lucide-react';
...
interface ClientAppProps {
  ...
  /** Coach materials visible to this client. */
  coachMaterials?: CoachMaterial[];
...
export const ClientApp: React.FC<ClientAppProps> = ({ clientProfile, allLessons, onLogout, onUpdateLesson, onSaveNewRecord, onDeleteLesson, onUpdateProfile, coachMaterials = [] }) => {
  const { t, language, setLanguage } = useLanguage();
  const [view, setView] = useState<ViewState | 'STATS' | 'PROFILE' | 'RESERVATION' | 'CALENDAR_INTEGRATION' | 'BAY_RESERVATION' | 'MY_BAY_RESERVATIONS' | 'POINT_PURCHASE' | 'PAYMENT_SUCCESS' | 'PAYMENT_FAIL' | 'RECENT_RECORDS' | 'AI_AGENT' | 'QUICK_LOG' | 'WEEKLY_INSIGHT' | 'MATERIALS' | 'COACH_FINDER'>(() => {
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            ...
                            <button 
                                ...
                                className="bg-yellow-400/30 hover:bg-yellow-400/50 backdrop-blur-sm text-white rounded-lg px-3 py-2.5 text-xs font-bold transition-all flex flex-col items-center justify-center gap-1 border border-yellow-300/40 hover:scale-[1.02]"
                            >
                            ...
                            <button 
                                ...
                                className="bg-indigo-500/40 hover:bg-indigo-500/60 backdrop-blur-sm text-white rounded-lg px-3 py-2.5 text-xs font-bold transition-all flex flex-col items-center justify-center gap-1 border border-indigo-300/40 hover:scale-[1.02]"
                            >
                            ...
                            <button 
                                ...
                                className="bg-emerald-600/40 hover:bg-emerald-600/60 backdrop-blur-sm text-white rounded-lg px-3 py-2.5 text-xs font-bold transition-all flex flex-col items-center justify-center gap-1 border border-emerald-400/40 hover:scale-[1.02]"
                            >
                            ...
                            <button 
                                ...
                                className="bg-purple-500/40 hover:bg-purple-500/60 backdrop-blur-sm text-white rounded-lg px-3 py-2.5 text-xs font-bold transition-all flex flex-col items-center justify-center gap-1 border border-purple-300/40 hover:scale-[1.02]"
                            >
                                <TrendingUp className="w-4 h-4 text-purple-200" />
                                주간 인사이트
                            </button>
                            <button 
                                onClick={() => { setSelectedLesson(null); setView('MATERIALS'); }}
                                className="bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white rounded-lg px-3 py-2.5 text-xs font-bold transition-all flex flex-col items-center justify-center gap-1 border border-white/20 hover:scale-[1.02]"
                            >
                            ...
                            <button 
                                onClick={() => { setSelectedLesson(null); setView('COACH_FINDER'); }}
                                className="bg-teal-500/40 hover:bg-teal-500/60 backdrop-blur-sm text-white rounded-lg px-3 py-2.5 text-xs font-bold transition-all flex flex-col items-center justify-center gap-1 border border-teal-300/40 hover:scale-[1.02]"
                            >
