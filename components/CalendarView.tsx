...
const getStatusStyling = (status: string): { typeLabel: string; bgColorClass: string; badgeColorClass: string } => {
  switch (status) {
    ...
    default:
      return {
        typeLabel: status,
        bgColorClass: 'bg-white/[0.06] border-line-subtle',
        badgeColorClass: 'bg-white/[0.20]',
      };
  }
};
...
interface CalendarViewProps {
  ...
  onDateClickRegister?: (date: string) => void; // Coach: start slot registration for a date
  /** Coach: open member reservation modal for a specific date + hour (timeGrid click) */
  onCoachCreateForMember?: (date: string, hour: number) => void;
}
...
const CalendarView: React.FC<CalendarViewProps> = ({ 
  ...
  const handleEventClick = async (clickInfo: any) => {
    ...
    // Coach calendar mode – do not allow cancellation from calendar click
  ...
  const handleDateClick = (dateInfo: DateClickArg) => {
    ...
    if (coachProfile) {
      // Time-grid click (dateStr contains 'T'): open member reservation modal
      if (clickedDate.includes('T') && onCoachCreateForMember) {
        const d = new Date(clickedDate);
        const date = getDateString(clickedDate);
        const hour = d.getHours();
        onCoachCreateForMember(date, hour);
        return;
      }

      // Day-grid click: navigate to slot registration (existing behaviour)
      if (onDateClickRegister) {
        onDateClickRegister(getDateString(clickedDate));
        return;
      }
    }

    const dateOnly = getDateString(clickedDate);
    ...
    <div className="relative p-2 sm:p-4">
      ...
      {showDateDetail && (
        <>
          ...
          {/* Side Panel */}
          <div className="fixed right-0 top-0 h-full w-full md:w-96 bg-white/[0.04] shadow-xl z-50 overflow-y-auto">
            {/* Header */}
            <div className="sticky top-0 bg-white/[0.04] border-b border-line-subtle px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-3 py-2 border border-line-subtle rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"
                    rows={3}
                  />
                </div>
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    onClick={() => setShowBookingModal(false)}
                    className="flex-1 px-4 py-2 border border-line-subtle text-ink-medium rounded-lg hover:bg-white/[0.03] transition"
                  >
                    취소
                  </button>
                  <button
