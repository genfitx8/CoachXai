...
import { Branch, Bay, CoachProfile, BayReservation } from '../types';
import {
...
export const CoachBayReservation: React.FC<CoachBayReservationProps> = ({
  ...
  const [confirmedReservation, setConfirmedReservation] = useState<BayReservation | null>(null);
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-4">
        ...
        {!loading && step === 'SELECT_SLOT' && (
          <div className="space-y-3">
            ...
            {timeSlots.map((slot) => {
              const hasPrice = slot.pricePoints !== null && slot.pricePoints !== undefined;
              const slotPrice = slot.pricePoints ?? 0;
              const affordable = hasPrice && (coachProfile.currentPoints ?? 0) >= slotPrice;
              return (
                <button
                  key={slot.startHour}
                  ...
                  className={`w-full text-left bg-white border rounded-xl px-4 py-3 transition-all ${
                    affordable
                      ? 'border-gray-200 hover:border-emerald-400 hover:bg-emerald-50 group'
                      : 'border-gray-100 opacity-50 cursor-not-allowed'
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <span className="font-medium text-gray-800">{formatHour(slot.startHour)}</span>
                    {hasPrice ? (
                      <span className={`text-sm font-bold ${affordable ? 'text-emerald-600' : 'text-red-400'}`}>
                        {slotPrice.toLocaleString()}pt
                      </span>
                    ) : (
                  ...
                  {hasPrice && !affordable && (
                    <div className="text-xs text-red-400 mt-0.5">포인트 부족</div>
                  )}
                </button>
              );
            })}
        ...
        {!loading && step === 'SELECT_BAY' && (
          <div className="space-y-3">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-3 text-sm">
              ...
              <span className="font-medium text-gray-700">{selectedDate && formatDate(selectedDate)}</span>
              <span className="mx-2 text-gray-300">|</span>
              <span className="font-medium text-gray-700">{selectedSlot && formatHour(selectedSlot.startHour)}</span>
            </div>
            ...
            {availableBays.map((entry) => (
              <button
                key={entry.bay.id}
                onClick={() => handleSelectBay(entry)}
                className="w-full text-left bg-white border border-gray-200 rounded-xl px-4 py-3 hover:border-emerald-400 hover:bg-emerald-50 transition-all group"
              >
                <div className="flex items-center justify-between">
                  <div>
                    <span className="font-semibold text-gray-900 group-hover:text-emerald-700">
                      {entry.bay.floor}층 {entry.bay.roomNumber}번 타석
                    </span>
                  </div>
                  <span className="text-sm font-bold text-emerald-600">{entry.pricePoints.toLocaleString()}pt</span>
                </div>
              </button>
            ))}
        ...
        {!loading && step === 'CONFIRM' && selectedBranch && selectedSlot && selectedBayEntry && (
          <div className="space-y-4">
            <h2 className="text-base font-bold text-gray-800">예약 확인</h2>
            <div className="bg-white border border-gray-200 rounded-xl divide-y divide-gray-100">
              ...
              <div className="px-4 py-3 flex justify-between text-sm">
                ...
                <span className="font-medium text-gray-900">{formatHour(selectedSlot.startHour)}</span>
              </div>
              <div className="px-4 py-3 flex justify-between text-sm">
                <span className="text-gray-500">타석</span>
              ...
              <div className="px-4 py-3 flex justify-between text-sm">
                ...
                <span className="font-bold text-red-500">-{selectedBayEntry.pricePoints.toLocaleString()}pt</span>
              </div>
              <div className="px-4 py-3 flex justify-between text-sm">
                <span className="text-gray-500">예약 후 잔액</span>
        ...
        {step === 'SUCCESS' && confirmedReservation && (
          <div className="space-y-4 text-center py-8">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">타석 예약이 성공적으로 접수되었습니다.</p>
            </div>
            <div className="bg-white border border-gray-200 rounded-xl divide-y divide-gray-100 text-left">
              ...
              <div className="px-4 py-3 flex justify-between text-sm">
                <span className="text-gray-500">시간</span>
                <span className="font-medium text-gray-900">{selectedSlot && formatHour(selectedSlot.startHour)}</span>
              </div>
              <div className="px-4 py-3 flex justify-between text-sm">
                <span className="text-gray-500">타석</span>
                <span className="font-medium text-gray-900">
