            ...
            <div className="bg-white rounded-xl shadow-sm border border-emerald-100 overflow-hidden">
              <div className="bg-emerald-50 px-4 py-3 border-b border-emerald-100 flex items-center justify-between">
                ...
                <div className="text-xs font-bold text-emerald-700">
                  {lesson.memberBodyAnalysis.bodyType} · {lesson.memberBodyAnalysis.swingType}
                </div>
              </div>
              <div className="p-5 space-y-4">
                <div className="grid sm:grid-cols-2 gap-3 text-sm">
                  {lesson.memberBodyAnalysis.structuralFactors.map((factor) => (
                    <div key={factor.name} className="border border-gray-100 rounded-lg p-3 bg-gray-50">
                      <div className="text-gray-500 text-xs">{factor.name}</div>
                      <div className="font-bold text-gray-800">{factor.value}</div>
                      <div className={`text-xs font-bold mt-1 ${factor.impact === '상' ? 'text-red-600' : factor.impact === '하' ? 'text-emerald-700' : 'text-gray-400'}`}>
                        영향도: {factor.impact}
                      </div>
                    </div>
                  ))}
                ...
                <div className="text-sm text-gray-600">
                  <span className="font-bold text-gray-800">코치 코멘트:</span>{' '}
                  {lesson.memberBodyAnalysis.coachComment || (
                    <span className="text-gray-400 italic">기록된 코멘트가 없습니다.</span>
                  )}
                </div>
              </div>
            </div>
