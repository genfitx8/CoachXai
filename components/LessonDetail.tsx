...
interface LessonDetailProps {
  ...
  coachId?: string; // Coach ID for permission checking
}
...
export const LessonDetail: React.FC<LessonDetailProps> = ({ lesson, allLessons = [], role = 'COACH', onBack, onUpdate, onDelete, onEdit, coachId }) => {
  ...
  // Permission for Edit/Delete buttons: Clients can edit/delete their own records, Coaches can edit/delete their own or assigned records
  ...
  return (
    <div className="fixed inset-0 z-50 bg-white flex flex-col h-full overflow-hidden animate-fade-in">
      ...
      {/* Header */}
      <div className="bg-gray-900 px-4 py-3 flex items-center justify-between text-white flex-shrink-0 safe-area-top relative">
                    ...
                    {activeMedia.type === 'video' && activeMedia.url && (
                        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent p-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                             <div className="flex items-center gap-3">
                                 ...
                                 <div className="flex-1 relative h-1.5 bg-gray-600 rounded-full cursor-pointer group/slider">
                                     <div 
                                 ...
                                 <button onClick={(e) => handleCaptureFrame('스냅샷', e)} className="text-white hover:text-emerald-400">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-blue-100 overflow-hidden">
                   <div className="bg-blue-600 px-4 py-3 flex justify-between items-center text-white">
                            ...
                            <div>
                                {isEditingScorecardDetail ? (
                                    <input
                                        ...
                                        className="bg-white/20 text-white font-bold px-2 py-1 rounded border-2 border-white/40 focus:border-white outline-none"
                                        placeholder="골프장 이름"
                                    />
                                ) : (
                                    <h3 className="font-bold">{lesson.scorecardDetail.courseName}</h3>
                                )}
                                <p className="text-[10px] text-blue-100">Detailed Scorecard</p>
                            </div>
                        </div>
                        <div className="text-right">
                             ...
                             <div className="text-[10px] text-blue-100 opacity-80">퍼팅 {isEditingScorecardDetail ? editingHoles.reduce((sum, h) => sum + h.putts, 0) : lesson.scorecardDetail.totalPutts}</div>
                             {canEdit && (
                                 <button
                                     onClick={() => {
                                     ...
                                     className="mt-2 bg-white/20 hover:bg-white/30 px-3 py-1 rounded-full text-xs font-bold transition-colors flex items-center gap-1"
                                 >
                                     {isEditingScorecardDetail ? (
