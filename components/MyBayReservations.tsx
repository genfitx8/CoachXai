...
> = {
  ...
  CANCELLED: {
    label: '취소됨',
    color: 'text-ink-muted bg-white/[0.03] border-line-subtle',
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-4">
        ...
        {!loading && reservations.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-ink-muted text-sm">예약 내역이 없습니다.</p>
          </div>
        )}
          ...
          <div className="space-y-3">
            ...
            {upcomingReservations.map((res) => {
              ...
              return (
                <div
                  key={res.id}
                  className="bg-white/[0.04] border border-line-subtle rounded-xl overflow-hidden"
                >
                  <div className="px-4 py-3 space-y-2">
                    ...
                    <div className="pt-1 flex items-center justify-between">
                      <span className="text-sm text-ink-muted">
                        차감 포인트: <span className="font-bold text-ink-medium">{res.paidPoints.toLocaleString()}pt</span>
                      </span>
                    </div>
                  </div>
                      ...
                      {!isConfirming ? (
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="text-xs text-ink-muted hover:text-ink-medium"
                          >
                            취소
                          </button>
                        </div>
          ...
          <div className="space-y-3">
            ...
            {pastReservations.map((res) => {
              ...
              return (
                <div
                  key={res.id}
                  className="bg-white/[0.04] border border-line-subtle rounded-xl px-4 py-3 opacity-70 space-y-2"
                >
                  <div className="flex items-center justify-between">
                    <span
                      className={`inline-flex items-center gap-1 text-xs font-semibold px-2 py-1 rounded-full border ${cfg.color}`}
