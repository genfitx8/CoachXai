...
type RecordType = 'PRACTICE' | 'SCORE' | 'LESSON' | 'BODY';
...
const LESSON_BODY_TYPES: LessonBodyType[] = [
  '이상체형',
  '삼각체형',
  '역삼각체형',
  '사각체형',
...
export const NewLessonForm: React.FC<NewLessonFormProps> = ({
  ...
  const [enableBodyAnalysis, setEnableBodyAnalysis] = useState(false);
  const [bodyType, setBodyType] = useState<LessonBodyType>('사각체형');
  const [bodyAnalysisComment, setBodyAnalysisComment] = useState('');
  const [frontAxisTiltDeg, setFrontAxisTiltDeg] = useState<string>('');
  const [headTiltDeg, setHeadTiltDeg] = useState<string>('');
  const [shoulderTiltDeg, setShoulderTiltDeg] = useState<string>('');
  const [pelvisTiltDeg, setPelvisTiltDeg] = useState<string>('');
  const [kneeTiltDeg, setKneeTiltDeg] = useState<string>('');
  ...
  const [frontBodyPhotoPreview, setFrontBodyPhotoPreview] = useState<string>('');
  const [sideBodyPhotoPreview, setSideBodyPhotoPreview] = useState<string>('');
  const [isAutoBodyAnalyzing, setIsAutoBodyAnalyzing] = useState(false);
  ...
  useEffect(() => {
    ...
    if (initialData) {
      ...
      if (initialData.memberBodyAnalysis) {
        setEnableBodyAnalysis(true);
        setBodyType(initialData.memberBodyAnalysis.bodyType);
        setBodyAnalysisComment(initialData.memberBodyAnalysis.coachComment || '');
        setFrontAxisTiltDeg(
          initialData.memberBodyAnalysis.structuralInput.frontAxisTiltDeg?.toString() || ''
        );
        setHeadTiltDeg(
          initialData.memberBodyAnalysis.structuralInput.headTiltDeg?.toString() || ''
        );
        setShoulderTiltDeg(
          initialData.memberBodyAnalysis.structuralInput.shoulderTiltDeg?.toString() || ''
        );
        setPelvisTiltDeg(
          initialData.memberBodyAnalysis.structuralInput.pelvisTiltDeg?.toString() || ''
        );
        setKneeTiltDeg(
          initialData.memberBodyAnalysis.structuralInput.kneeTiltDeg?.toString() || ''
        );
      }
  ...
  }, [frontBodyPhotoPreview, sideBodyPhotoPreview]);
  ...
  const handleSelectType = (type: RecordType) => {
    ...
    if (type === 'SCORE') {
      ...
      setTitle(`${dateStr} 신체 데이터 기록`);
      setIsDataExtractionMode(false);
    } else {
      setTitle(`${dateStr} 자율 연습`);
    }

    setEnableBodyAnalysis(type === 'BODY');
  };
  ...
  const parseMetric = (raw: string): number | undefined => {
    if (raw.trim() === '') return undefined;
    const parsed = Number(raw);
    return Number.isFinite(parsed) ? parsed : undefined;
  };

  const handleBodyPhotoChange = (
    position: 'front' | 'side',
    ...
    if (position === 'front') {
      ...
      setFrontBodyPhotoPreview(previewUrl);
    } else {
      if (sideBodyPhotoPreview) URL.revokeObjectURL(sideBodyPhotoPreview);
      setSideBodyPhoto(file);
      setSideBodyPhotoPreview(previewUrl);
    }
  };

  const handleAutoBodyAnalysis = async () => {
    if (!frontBodyPhoto || !sideBodyPhoto) {
      setError('정면/측면 전신 사진을 모두 업로드해주세요.');
      return;
    }
    setError(null);
    setIsAutoBodyAnalyzing(true);

    try {
      const result = await analyzeBodyPhotos({
        frontImage: { data: frontBodyPhoto, mimeType: frontBodyPhoto.type },
        sideImage: { data: sideBodyPhoto, mimeType: sideBodyPhoto.type },
      });

      setBodyType(result.bodyType);
      setFrontAxisTiltDeg(result.structuralInput.frontAxisTiltDeg?.toString() || '');
      setHeadTiltDeg(result.structuralInput.headTiltDeg?.toString() || '');
      setShoulderTiltDeg(result.structuralInput.shoulderTiltDeg?.toString() || '');
      setPelvisTiltDeg(result.structuralInput.pelvisTiltDeg?.toString() || '');
      setKneeTiltDeg(result.structuralInput.kneeTiltDeg?.toString() || '');
      setBodyAnalysisComment(result.coachComment);
    } catch (err) {
      console.error(err);
      setError('신체 사진 자동 분석에 실패했습니다. 수동 입력으로 진행해주세요.');
    } finally {
      setIsAutoBodyAnalyzing(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if ((recordType !== 'SCORE' || scoreMode === 'SIMPLE') && recordType !== 'BODY') {
    ...
    try {
      ...
      else if (recordType === 'BODY') tags.push('신체데이터', '체형기록');
      ...
      let memberBodyAnalysis: LessonBodyAnalysis | undefined = initialData?.memberBodyAnalysis;
      if (enableBodyAnalysis) {
        const structuralInput = {
          frontAxisTiltDeg: parseMetric(frontAxisTiltDeg),
          headTiltDeg: parseMetric(headTiltDeg),
          shoulderTiltDeg: parseMetric(shoulderTiltDeg),
          pelvisTiltDeg: parseMetric(pelvisTiltDeg),
          kneeTiltDeg: parseMetric(kneeTiltDeg),
        };
        memberBodyAnalysis = {
          bodyType,
          ...
          coachComment: bodyAnalysisComment.trim() || undefined,
        };
        tags.push('신체분석');
      } else if (initialData && !enableBodyAnalysis) {
        memberBodyAnalysis = undefined;
      }
      ...
      <div className="max-w-md mx-auto bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden animate-fade-in-up">
        ...
        <div className="p-6 space-y-4">
          ...
          <button
            ...
            className="w-full flex items-center p-4 border-2 border-gray-100 rounded-xl hover:border-emerald-600 hover:bg-emerald-50 transition-all group text-left"
          >
            <div className="bg-emerald-100 p-3 rounded-full text-emerald-600 group-hover:bg-emerald-200 group-hover:text-emerald-800 mr-4">
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden animate-fade-in">
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        ...
        {recordType !== 'BODY' && (recordType !== 'SCORE' || scoreMode === 'SIMPLE') && (
          <>
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {recordType !== 'SCORE' && recordType !== 'BODY' && (
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2">
                사용 클럽
              </label>
        ...
        {recordType === 'BODY' && (
        <div className="bg-white border border-emerald-100 rounded-xl p-4 space-y-4">
          <div
            onClick={() => setEnableBodyAnalysis((prev) => !prev)}
          ...
          >
            <ScanLine className={`w-5 h-5 ${enableBodyAnalysis ? 'text-emerald-700' : 'text-gray-400'}`} />
            <div>
              <h4 className="font-bold text-sm text-gray-900">회원 신체 분석</h4>
              <p className="text-xs text-gray-500">체형과 정렬 데이터를 함께 기록합니다.</p>
            </div>
            {enableBodyAnalysis && (
          ...
          {enableBodyAnalysis && (
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <label className="border border-dashed border-gray-300 rounded-lg p-3 text-xs text-gray-600 cursor-pointer hover:border-emerald-500 transition-colors">
                  <span className="font-bold text-gray-700 block mb-2">정면 전신 사진 업로드</span>
                  <input type="file" accept="image/*" className="hidden" onChange={(e) => handleBodyPhotoChange('front', e)} />
                  {frontBodyPhotoPreview ? (
                    <img src={frontBodyPhotoPreview} alt="정면 전신 미리보기" className="w-full h-32 object-cover rounded-md" />
                  ) : (
                    <span className="text-gray-400">클릭해서 이미지 선택</span>
                  )}
                </label>
                <label className="border border-dashed border-gray-300 rounded-lg p-3 text-xs text-gray-600 cursor-pointer hover:border-emerald-500 transition-colors">
                  <span className="font-bold text-gray-700 block mb-2">측면 전신 사진 업로드</span>
                  <input type="file" accept="image/*" className="hidden" onChange={(e) => handleBodyPhotoChange('side', e)} />
                  {sideBodyPhotoPreview ? (
                    <img src={sideBodyPhotoPreview} alt="측면 전신 미리보기" className="w-full h-32 object-cover rounded-md" />
                  ) : (
                    <span className="text-gray-400">클릭해서 이미지 선택</span>
                  )}
                </label>
              ...
              <Button
                type="button"
                onClick={handleAutoBodyAnalysis}
                disabled={isAutoBodyAnalyzing || !frontBodyPhoto || !sideBodyPhoto}
                className="w-full bg-emerald-700 hover:bg-emerald-800 text-white text-sm py-2.5"
              >
                {isAutoBodyAnalyzing ? '신체 자동 분석 중...' : '사진으로 자동 분석 후 입력'}
              </Button>
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  추천 스윙 유형: <span className="font-bold text-emerald-700">{inferSwingTypeFromBodyType(bodyType)}</span>
                </p>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <input value={frontAxisTiltDeg} onChange={(e) => setFrontAxisTiltDeg(e.target.value)} type="number" step="0.1" placeholder="정면 축 기울기(°)" className="px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-700 outline-none" />
                <input value={headTiltDeg} onChange={(e) => setHeadTiltDeg(e.target.value)} type="number" step="0.1" placeholder="머리 기울기(°)" className="px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-700 outline-none" />
                <input value={shoulderTiltDeg} onChange={(e) => setShoulderTiltDeg(e.target.value)} type="number" step="0.1" placeholder="어깨 기울기(°)" className="px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-700 outline-none" />
                <input value={pelvisTiltDeg} onChange={(e) => setPelvisTiltDeg(e.target.value)} type="number" step="0.1" placeholder="골반 기울기(°)" className="px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-700 outline-none" />
                <input value={kneeTiltDeg} onChange={(e) => setKneeTiltDeg(e.target.value)} type="number" step="0.1" placeholder="무릎 기울기(°)" className="px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-700 outline-none sm:col-span-2" />
              </div>

              <textarea
                value={bodyAnalysisComment}
                onChange={(e) => setBodyAnalysisComment(e.target.value)}
                rows={2}
                placeholder="예: 골반 좌측 하강으로 다운스윙 시 체중이동 지연됨"
                className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-700 outline-none resize-none"
              />
            </div>
          )}
        </div>
