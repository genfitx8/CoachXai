...
export const NewLessonForm: React.FC<NewLessonFormProps> = ({
  ...
  const [inputMethod, setInputMethod] = useState<'upload' | 'camera' | 'voice'>(
    'upload'
  );
  ...
  const handleStartLesson = () => {
    ...
    if (!isExistingClientSelected && !clientPhone.trim()) {
      setError('등록되지 않은 회원입니다. 전화번호를 입력해주세요.');
      return;
    }
  ...
  const processFile = (file: File) => {
    ...
    if (file.size > MAX_SIZE) {
      setError('파일 크기가 너무 큽니다. (5GB 이하 권장)');
      return;
    }
  ...
  const addHomework = () => {
    ...
    if (hwDays.length === 0) {
      setError('요일을 최소 하루 이상 선택해주세요.');
      return;
    }

    const newBatch = generateHomeworkBatch();
    if (newBatch.length === 0) {
      setError('생성된 과제가 없습니다. 날짜와 기간을 확인해주세요.');
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      ...
      setError('AI 샷 데이터 분석을 위해 사용한 클럽을 선택해주세요.');
      return;
    }
    ...
    try {
      ...
      // Logic for SIMPLE Scorecard / Other Modes
        ...
        try {
          if (isDataExtractionMode && mainMedia.type === 'image') {
            setStatusMessage(
              recordType === 'SCORE'
            ...
            const dataResult = await extractGolfData(
              { data: mainMedia.file, mimeType: mainMedia.file.type },
              nameToSearch
            );
  ...
  if (step === 'PACKAGE_SELECT') {
    ...
    const handleConfirmSession = () => {
      if (!selectedPackageId || selectedSessionNumber === null) {
        setError('패키지와 회차를 선택하거나 "패키지 없이 기록하기"를 선택해주세요.');
        return;
      }
      setError(null);
      setStep('FORM');
        ...
        <div className="bg-emerald-600 px-6 py-4 flex justify-between items-center text-white">
          <div className="flex items-center gap-2">
            <button
              ...
              aria-label="회원 선택으로 돌아가기"
            >
              <ArrowLeft className="w-5 h-5" />
            </button>
            <h2 className="text-lg font-bold flex items-center gap-2">
              <BookOpen className="w-5 h-5" /> 레슨 패키지 선택
            </h2>
          </div>
      ...
      <div className="max-w-md mx-auto bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden animate-fade-in-up">
        ...
        <div className="p-6 space-y-5">
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition-all"
              />
            </div>
            {/* Suggestion List */}
            {clientName.trim() &&
          ...
          <Button
            ...
            className="w-full py-4 text-lg font-bold shadow-lg shadow-emerald-200 mt-4"
          >
      ...
      <div className="max-w-md mx-auto bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden animate-fade-in-up">
        ...
        <div className="p-6 space-y-4">
          ...
          <button
            ...
            className="w-full flex items-center p-4 border-2 border-gray-100 rounded-xl hover:border-emerald-500 hover:bg-emerald-50 transition-all group text-left"
          >
    ...
    <div className="max-w-2xl mx-auto bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden animate-fade-in">
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              {showAddInterface ? (
                <div className="border-2 border-dashed border-gray-300 rounded-2xl p-8 bg-gray-50 hover:bg-gray-100 transition-colors">
                  ...
                  {inputMethod === 'upload' && (
                    ...
                    >
                      <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center mx-auto mb-4 shadow-sm text-emerald-500 cursor-pointer">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    ...
                    <button
                      ...
                      className="aspect-square rounded-xl border-2 border-dashed border-gray-300 flex flex-col items-center justify-center text-gray-400 hover:text-emerald-500 hover:border-emerald-300 hover:bg-emerald-50 transition-all gap-1"
                    >
        ...
        <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100 space-y-4">
          ...
          <div className="flex gap-2">
            ...
            <button
              type="button"
              onClick={addHomework}
              className="bg-indigo-600 text-white px-3 py-2 rounded-lg text-sm font-bold hover:bg-indigo-700 transition-colors"
            >
              추가
            </button>
          </div>
        ...
        <div className="pt-4 border-t border-gray-100 flex gap-3">
          ...
          <Button
            type="submit"
            className="flex-[2] py-3 text-lg font-bold shadow-lg shadow-emerald-200"
