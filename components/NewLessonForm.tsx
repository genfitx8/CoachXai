...
export const NewLessonForm: React.FC<NewLessonFormProps> = ({
  ...
  const [bodyPhotoInputMethod, setBodyPhotoInputMethod] = useState<'camera' | 'upload'>('camera');
  ...
  const handleAutoBodyAnalysis = async () => {
    if (!frontBodyPhoto || !sideBodyPhoto) {
      setError('정면/측면 전신 사진을 모두 촬영하거나 업로드해주세요.');
      return;
    }
    setError(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (recordType !== 'SCORE' || scoreMode === 'SIMPLE') {
      if (mediaItems.length === 0 && !hasBodyAnalysisPhoto) {
    ...
    <div className="max-w-2xl mx-auto bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden animate-fade-in">
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        ...
        <div className="bg-white border border-emerald-100 rounded-xl p-4 space-y-4">
          ...
          {enableBodyAnalysis && (
            <div className="space-y-4">
              <div className="inline-flex rounded-lg border border-emerald-200 p-1 bg-emerald-50">
                <button
                  type="button"
                  onClick={() => setBodyPhotoInputMethod('camera')}
                  className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-colors ${
                    bodyPhotoInputMethod === 'camera'
                ...
                <button
                  type="button"
                  onClick={() => setBodyPhotoInputMethod('upload')}
                  className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-colors ${
                    bodyPhotoInputMethod === 'upload'
                      ? 'bg-emerald-700 text-white'
                      : 'text-emerald-700 hover:bg-emerald-100'
                  }`}
                >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <label className="border border-dashed border-gray-300 rounded-lg p-3 text-xs text-gray-600 cursor-pointer hover:border-emerald-500 transition-colors">
                  <span className="font-bold text-gray-700 block mb-2">
                    정면 전신 사진 {bodyPhotoInputMethod === 'camera' ? '촬영' : '업로드'}
                  </span>
                  <input
                    type="file"
                    accept="image/*"
                    capture={bodyPhotoInputMethod === 'camera' ? 'environment' : undefined}
                    className="hidden"
                    onChange={(e) => handleBodyPhotoChange('front', e)}
                  />
                  {frontBodyPhotoPreview ? (
                    <img src={frontBodyPhotoPreview} alt="정면 전신 미리보기" className="w-full h-32 object-cover rounded-md" />
                  ) : (
                    <span className="text-gray-400">
                      {bodyPhotoInputMethod === 'camera' ? '클릭해서 바로 촬영' : '클릭해서 이미지 선택'}
                    </span>
                  )}
                </label>
                <label className="border border-dashed border-gray-300 rounded-lg p-3 text-xs text-gray-600 cursor-pointer hover:border-emerald-500 transition-colors">
                  <span className="font-bold text-gray-700 block mb-2">
                    측면 전신 사진 {bodyPhotoInputMethod === 'camera' ? '촬영' : '업로드'}
                  </span>
                  <input
                    type="file"
                    accept="image/*"
                    capture={bodyPhotoInputMethod === 'camera' ? 'environment' : undefined}
                    className="hidden"
                    onChange={(e) => handleBodyPhotoChange('side', e)}
                  />
                  {sideBodyPhotoPreview ? (
