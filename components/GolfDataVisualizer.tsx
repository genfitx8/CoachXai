...
export const GolfDataVisualizer: React.FC<GolfDataVisualizerProps> = ({ 
  ...
  const renderMetricCard = (label: string, value: number | undefined | null, unit: string, color: string) => (
    <div className="bg-white/[0.03] p-4 rounded-xl border border-line-subtle flex flex-col items-center justify-center">
      <span className="text-ink-muted text-xs font-medium uppercase mb-1">{label}</span>
      <div className={`text-2xl font-bold ${color}`}>
        {value != null ? value : '-'} <span className="text-sm font-normal text-ink-muted">{unit}</span>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      {/* Current Data Table */}
      <div className="bg-white/[0.04] rounded-xl shadow-sm border border-line-subtle overflow-hidden">
        <div className="bg-base px-6 py-4 flex justify-between items-center">
        ...
        <div className="p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                 <div className="flex justify-between items-center p-3 border-b border-line-subtle">
                    <span className="text-ink-muted text-sm">발사각</span>
                    <span className="font-bold text-ink-high">{currentData.launchAngle ?? '-'}°</span>
                 </div>
                 <div className="flex justify-between items-center p-3 border-b border-line-subtle">
                    <span className="text-ink-muted text-sm">백스핀</span>
                    <span className="font-bold text-ink-high">{currentData.backSpin ?? currentData.spinRate ?? '-'} rpm</span>
                 </div>
                 <div className="flex justify-between items-center p-3 border-b border-line-subtle">
                    <span className="text-ink-muted text-sm">사이드스핀</span>
                    <span className="font-bold text-ink-high">{currentData.sideSpin ?? '-'} rpm</span>
                 </div>
                 <div className="flex justify-between items-center p-3 border-b border-line-subtle">
                    <span className="text-ink-muted text-sm">정타율</span>
                    <span className="font-bold text-ink-high">{currentData.smashFactor ?? '-'}</span>
                 </div>
                 <div className="flex justify-between items-center p-3 border-b border-line-subtle">
                    <span className="text-ink-muted text-sm">어택 앵글</span>
                    <span className="font-bold text-ink-high">{currentData.attackAngle ?? '-'}°</span>
                 </div>
                 <div className="flex justify-between items-center p-3 border-b border-line-subtle">
                    <span className="text-ink-muted text-sm">클럽 패스</span>
                    <span className="font-bold text-ink-high">{currentData.clubPath ?? '-'}°</span>
                 </div>
                 <div className="flex justify-between items-center p-3 border-b border-line-subtle">
                    <span className="text-ink-muted text-sm">다이나믹 로프트</span>
                    <span className="font-bold text-ink-high">{currentData.dynamicLoft ?? '-'}°</span>
                 </div>
                 <div className="flex justify-between items-center p-3 border-b border-line-subtle">
                    <span className="text-ink-muted text-sm">사이드 토탈</span>
                    <span className="font-bold text-ink-high">{currentData.sideTotal ?? '-'} m</span>
                 </div>
            </div>
        </div>
      </div>
      ...
      {historyData.length > 1 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
             {/* Distance Chart */}
             <div className="bg-white/[0.04] p-6 rounded-xl shadow-sm border border-line-subtle">
