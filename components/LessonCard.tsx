...
export const LessonCard: React.FC<LessonCardProps> = ({ lesson, onClick, onShare, onDelete, showMedia = true }) => {
  ...
  const renderBadge = () => {
      if (!isSelfRecord) {
          return (
             <div className="absolute top-2 left-2 z-20 px-2.5 py-1 rounded-lg text-xs font-bold shadow-lg flex items-center gap-1 bg-gradient-to-r from-emerald-600 to-emerald-700 text-white border border-white/20">
    ...
    >
      {showMedia ? (
        <div className="relative aspect-[9/16] bg-gray-200 flex items-center justify-center overflow-hidden">
          ...
          {lesson.feedbackStatus === 'COMPLETED' && (
               <div className="absolute top-2 right-2 z-30 px-2.5 py-1 rounded-lg text-xs font-bold shadow-lg flex items-center gap-1 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white border border-white/20">
        ...
        <div className="relative h-12 bg-gradient-to-r from-gray-50 to-gray-100 flex items-center justify-center border-b border-gray-200">
          <span className="text-xs text-gray-400 font-medium">미디어 숨김</span>
        </div>
      )}
      ...
      <div className="p-4 flex-1 flex flex-col">
        ...
        <div className="flex items-center text-xs text-gray-400 mb-1.5">
          ...
          <span className="font-medium">{lesson.clientName || '회원'}</span>
        </div>
        ...
        <div className="mt-auto pt-3 border-t border-gray-100 flex items-center justify-between">
            <div className="flex items-center text-xs font-semibold text-emerald-600 group-hover:text-emerald-700 transition-colors">
                자세히 보기 <ChevronRight className="w-3.5 h-3.5 ml-0.5 group-hover:translate-x-1 transition-transform" />
            </div>
