...
describe('LessonPackageManager', () => {
  it('shows an empty-state message when no packages exist', () => {
    renderManager();
    expect(screen.getByText(/등록된 패키지가 없습니다/i)).toBeInTheDocument();
  });

  it('shows a "새 레슨 패키지 등록" button', () => {
    renderManager();
    expect(screen.getByRole('button', { name: /패키지 생성/i })).toBeInTheDocument();
  });

  it('opens the new-package modal when the button is clicked', () => {
    renderManager();
    fireEvent.click(screen.getByRole('button', { name: /패키지 생성/i }));
    // The modal header should appear
    expect(screen.getByRole('heading', { name: /새 패키지 생성/i })).toBeInTheDocument();
