...
describe('ClientReservation – date-based time slot selection', () => {
  ...
  it('loads available slots for the selected date when date changes', async () => {
    vi.mocked(reservationService.getAvailableSlots).mockResolvedValue([SLOT_A, SLOT_B]);
  ...
  it('deselects a slot when clicked a second time', async () => {
    vi.mocked(reservationService.getAvailableSlots).mockResolvedValue([SLOT_A]);
  ...
  it('refreshes slot list when date is changed to a different date', async () => {
    vi.mocked(reservationService.getAvailableSlots)
      .mockResolvedValueOnce([])     // initial loadAvailableSlots (useEffect on mount)
