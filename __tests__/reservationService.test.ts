...
// ─── requestReservation triggers in-app notification ─────────────────────────

describe('reservationService – requestReservation creates in-app notification', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    (firebaseService.isInitialized as any).mockReturnValue(false);
    (storageService.updateReservation as any).mockReturnValue(undefined);
    (sendLessonReservationNotifications as any).mockResolvedValue(undefined);
    (coachNotificationService.createReservationRequestNotification as any).mockResolvedValue(undefined);
  });

  it('calls createReservationRequestNotification after requestReservation', async () => {
    (storageService.getReservations as any).mockReturnValue([
      makeReservation({ id: 'res-available', status: 'AVAILABLE' }),
    ]);
  ...
  it('calls createReservationRequestNotification after requestReservationWithTime', async () => {
    (storageService.getReservations as any).mockReturnValue([]);
    (storageService.saveReservation as any).mockReturnValue(undefined);
...
// ─── createCoachMadeLessonReservation ─────────────────────────────────────────

describe('reservationService – createCoachMadeLessonReservation', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    (firebaseService.isInitialized as any).mockReturnValue(false);
    (storageService.getReservations as any).mockReturnValue([]);
  ...
  it('creates a CONFIRMED reservation directly for the member (storage mode)', async () => {
    const result = await reservationService.createCoachMadeLessonReservation(
      'coach1',
      '박코치',
      'member1_010-0000-0000',
      '김회원',
      '010-0000-0000',
      '2026-05-01T10:00:00',
      '2026-05-01T11:00:00',
    ...
    expect(result.coachId).toBe('coach1');
    expect(result.clientName).toBe('김회원');
    expect(result.clientPhone).toBe('010-0000-0000');
    expect(result.notes).toBe('전화 예약');
    expect(result.createdByCoachId).toBe('coach1');
    expect(storageService.saveReservation).toHaveBeenCalledWith(
      expect.objectContaining({ status: 'CONFIRMED', clientName: '김회원' })
    );
  });

  it('throws when there is a CONFIRMED conflict at the same time', async () => {
    (storageService.getReservations as any).mockReturnValue([
      makeReservation({
        coachId: 'coach1',
        status: 'CONFIRMED',
    ...
    ).rejects.toThrow('이미 다른 예약이 있습니다');
  });

  it('throws when there is a BLOCKED conflict at the same time', async () => {
    (storageService.getReservations as any).mockReturnValue([
      makeReservation({
        coachId: 'coach1',
        status: 'BLOCKED',
    ...
    ).rejects.toThrow('예약이 불가능합니다');
  });

  it('does NOT conflict with CANCELLED reservations', async () => {
    (storageService.getReservations as any).mockReturnValue([
      makeReservation({
        coachId: 'coach1',
        status: 'CANCELLED',
  ...
  it('creates reservation in Firebase mode', async () => {
    (firebaseService.isInitialized as any).mockReturnValue(true);
    (firebaseService.getReservations as any).mockResolvedValue([]);
    (firebaseService.saveReservation as any).mockResolvedValue(undefined);
    ...
    expect(result.status).toBe('CONFIRMED');
    expect(firebaseService.saveReservation).toHaveBeenCalledWith(
