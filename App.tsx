...
const AppContent: React.FC = () => {
  ...
  return (
    <div className="sn-app-bg min-h-screen flex flex-col font-sans text-slate-900">
      {/* Header */}
      <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/85 shadow-sm backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 h-16 md:h-[4.5rem] flex items-center justify-between gap-2">
          <div
          ...
          >
            <div className="bg-slate-900 p-2 rounded-xl shadow-sm text-white">
              <Menu className="w-5 h-5" />
            </div>
            <span className="font-bold text-lg md:text-xl text-slate-900 tracking-tight">
              SwingNote
            </span>
            <span className="bg-slate-100 text-slate-600 text-[10px] px-2 py-0.5 rounded-full font-bold uppercase border border-slate-200/90">
              {t('coach')}
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-3">
            <button
              ...
              className="flex items-center gap-1 text-xs md:text-sm font-semibold text-slate-500 hover:text-slate-700 transition-colors bg-slate-100/90 px-2.5 py-1.5 rounded-xl border border-slate-200"
            >
            ...
            {currentUser && 'id' in currentUser && (
              <div
                className="hidden sm:flex items-center gap-2 text-sm text-slate-600 hover:bg-slate-100 px-3 py-1.5 rounded-full cursor-pointer transition-colors border border-transparent hover:border-slate-200"
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 py-5 md:py-7">
        {coachView === 'LIST' && (
          <div className="space-y-5 md:space-y-6 animate-fade-in">
            ...
            <button
              ...
              className="w-full relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-slate-900 to-slate-800 px-5 py-4 md:p-5 flex items-center gap-4 shadow-lg shadow-slate-900/20 group hover:-translate-y-0.5 hover:shadow-xl hover:shadow-slate-900/30 transition-all duration-300"
            ...
            <div className="flex items-center justify-between px-1">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1">{t('coach_dashboard_desc')}</p>
              </div>
            </div>

            {/* ── Lesson-Record Stats Row ────────────────────────────────── */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5" data-testid="dashboard-stats">
              <div className="sn-surface rounded-2xl p-4 md:p-5 flex items-start gap-3">
                <div className="w-10 h-10 bg-slate-100 rounded-xl flex items-center justify-center flex-shrink-0">
                  <BookOpen className="w-4 h-4 text-slate-600" />
                </div>
                <div>
                  <div className="text-2xl font-bold text-slate-800">{allCoachLessons.length}</div>
                  <div className="text-xs font-medium text-slate-500 mt-1">전체 레슨 기록</div>
                </div>
              </div>
              <div className="sn-surface rounded-2xl p-4 md:p-5 flex items-start gap-3">
                <div className="w-10 h-10 bg-slate-100 rounded-xl flex items-center justify-center flex-shrink-0">
                ...
                <div>
                  <div className="text-2xl font-bold text-slate-800">{dashboardData.todayLessons.length}</div>
                  <div className="text-xs font-medium text-slate-500 mt-1">오늘 레슨</div>
                </div>
              </div>
              <div className="sn-surface rounded-2xl p-4 md:p-5 flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs font-medium text-amber-500 mt-1">미완료 패키지</div>
                </div>
              </div>
              <div className="sn-surface rounded-2xl p-4 md:p-5 flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs font-medium text-emerald-700 mt-1">담당 회원</div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              <Button
                ...
                className="w-full py-4 text-base justify-center rounded-2xl bg-slate-900 hover:bg-slate-800"
              ...
              <Button
                ...
                className="w-full py-4 text-base justify-center rounded-2xl bg-emerald-700 hover:bg-emerald-600"
            ...
            {dashboardData.incompletePackages.length > 0 && (
              <div className="sn-surface rounded-2xl p-5">
            ...
            {dashboardData.todayLessons.length > 0 && (
              <div className="sn-muted-surface rounded-2xl p-5">
            ...
            <div className="sn-surface rounded-2xl p-5">
              <p className="sn-eyebrow text-xs font-semibold text-slate-400 mb-4">예약 및 회원 관리</p>
        ...
        {coachView === 'LESSON_LIST' && (
          <div className="space-y-5 md:space-y-6 animate-fade-in">
            {/* Back to Dashboard */}
            <div className="sn-surface rounded-2xl p-4 md:p-5 flex flex-col sm:flex-row sm:items-center gap-3">
              <button
                onClick={() => setCoachView('LIST')}
                className="flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-slate-800 transition-colors px-3 py-2 rounded-xl hover:bg-slate-100"
              >
                <X className="w-4 h-4" />
                대시보드로 돌아가기
              </button>
            ...
            <div className="sn-surface rounded-2xl p-4 md:p-5 space-y-4">
              {/* Client Filter Section */}
              {userRole === 'COACH' && clients.length > 0 && (
                <div className="flex flex-col lg:flex-row lg:items-center gap-3">
                  <div className="flex items-center gap-2 text-slate-700 lg:w-28">
                    <User className="w-4 h-4 text-slate-500" />
                    <span className="text-sm font-semibold">회원 필터</span>
                  </div>
                  <div className="flex-1">
                    <select
                      value={selectedClientFilter}
                      onChange={(e) => setSelectedClientFilter(e.target.value)}
                      className="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-slate-900/15 focus:border-slate-400 outline-none font-medium text-slate-700 bg-white"
                    >
                  ...
                  {selectedClientFilter && (
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="text-sm text-slate-500 hover:text-red-500 flex items-center gap-1 px-3 py-2 rounded-xl hover:bg-red-50 transition-colors"
                      >
                        <X className="w-4 h-4" />
                        초기화
                      </button>
              ...
              <div className="flex items-center justify-between bg-slate-50 rounded-xl border border-slate-200 px-4 py-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-slate-700 text-sm">레슨 미디어 표시</span>
                </div>
                <button
              ...
              {selectedClientFilter && (
                <div className="text-sm text-slate-600">
                  <span className="font-bold text-slate-800">{selectedClientFilter}</span>님의 레슨 {filteredLessons.length}개
                </div>
              )}
            </div>
            ...
            {filteredLessons.length === 0 ? (
              <div className="text-center py-16 md:py-20 sn-surface rounded-2xl border border-dashed border-slate-300/90">
                ...
                <p className="text-slate-500 mt-1">{t('no_lessons_desc')}</p>
              </div>
            ) : (
