...
const AppContent: React.FC = () => {
  ...
  // Coach Material State
  const [coachMaterials, setCoachMaterials] = useState<CoachMaterial[]>([]);
  const [selectedClientForMaterial, setSelectedClientForMaterial] = useState<ClientProfile | null>(null);
  ...
  const loadData = async (useFirebase: boolean) => {
    ...
    if (currentCoachId) {
      ...
      // Load coach materials
      if (useFirebase) {
        try {
          const materials = await firebaseService.getCoachMaterialsByCoach(currentCoachId);
          setCoachMaterials(materials);
        } catch {
          setCoachMaterials(storageService.getCoachMaterials());
        }
      } else {
        setCoachMaterials(storageService.getCoachMaterials());
      }
  ...
  const handleSaveCoachMaterial = async (material: CoachMaterial) => {
    setCoachMaterials((prev) => {
      const idx = prev.findIndex((m) => m.id === material.id);
      return idx >= 0 ? [...prev.slice(0, idx), material, ...prev.slice(idx + 1)] : [...prev, material];
    });
    if (firebaseService.isInitialized()) {
      try {
        await firebaseService.saveCoachMaterial(material);
      } catch (e) {
        console.error('Failed to save coach material to Firebase:', e);
  ...
  const handleDeleteCoachMaterial = async (materialId: string) => {
    setCoachMaterials((prev) => prev.filter((m) => m.id !== materialId));
    if (firebaseService.isInitialized()) {
      try {
        await firebaseService.deleteCoachMaterial(materialId);
      } catch (e) {
        console.error('Failed to delete coach material from Firebase:', e);
        storageService.deleteCoachMaterial(materialId);
      }
    } else {
      storageService.deleteCoachMaterial(materialId);
    }
  ...
  if (userRole === 'CLIENT' && currentUser && !('id' in currentUser)) {
    const clientId = `${currentUser.name}_${currentUser.phone}`;
    const clientVisibleMaterials = coachMaterials.filter(
      (m) => m.clientId === clientId && m.status === 'published'
    );
    return (
      <ClientApp
        ...
        coachMaterials={clientVisibleMaterials}
      />
    );
  }
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 py-6">
        ...
        {coachView === 'CLIENTS' && (
          <CoachClientManager
            ...
            onGenerateMaterial={(client) => {
              setSelectedClientForMaterial(client);
              setCoachView('COACH_MATERIAL');
            }}
          />
        )}
        ...
        {coachView === 'COACH_MATERIAL' && selectedClientForMaterial && currentUser && 'id' in currentUser && (
          <CoachMaterialGenerator
            client={selectedClientForMaterial}
            lessons={lessons}
            coachId={(currentUser as CoachProfile).id}
            materials={coachMaterials}
            onBack={() => {
              setCoachView('CLIENTS');
              setSelectedClientForMaterial(null);
            }}
            onSaveMaterial={handleSaveCoachMaterial}
            onDeleteMaterial={handleDeleteCoachMaterial}
          />
        )}
      </main>
